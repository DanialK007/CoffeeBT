...
if (isset($_POST["create"])) {
    ...
    $targetFile = $targetDirectory . basename($_FILES["menu_img"]["name"]);

    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

    // Check if image file is a actual image or fake image
    $check = getimagesize($_FILES["menu_img"]["tmp_name"]);
    if ($check !== false) {
        $uploadOk = 1;
        ...
        if (file_exists($targetFile)) {
            ...
            if ($_FILES["menu_img"]["size"] > 5000000) {
                ...
                if (!in_array($imageFileType, $allowedExtensions)) {
                    ...
                    if (move_uploaded_file($_FILES["menu_img"]["tmp_name"], $targetFile)) {
                        // echo "The file " . basename($_FILES["menu_img"]["name"]) . " has been uploaded.";
                        $menu_img = $targetFile;  // Store this path in your database
                    } else {
    ...
    if (isset($_FILES["menu_img"]) && $_FILES["menu_img"]["error"] == 0) {
        ...
        // Insert into the database with $menu_img (assuming you have modified your table structure to include menu_img)
        $sql = "INSERT INTO `menu` (menu_name, menu_price, menu_img)
        VALUES ('$menu_name', '$menu_price', '$menu_img')";
...
<div class="container pt-3">
    ...
    <div class="display-3 pb-2 fw-bold">Menu</div>
            ...
            <form action="" method="post" class="modal-content" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <h1 class="py-3">Create a New menu</h1>
                            <div class="row p-2">
                                <div class="col-4">
                                    <label for="form-label" class="fs-5 pb-2">menu Name</label>
                                </div>
                                <div class="col-8">
                                    <input required type="text" name="menu_name" class="form-control p-3">
                                </div>
                            </div>
                            <div class="row p-2">
                                <div class="col-4">
                                    <label for="form-label" class="fs-5 pb-2">menu price</label>
                                </div>
                                <div class="col-8">
                                    <input required type="text" name="menu_price" class="form-control p-3">
                                </div>
                            </div>
                            <div class="row p-2">
                                <div class="col-4">
                                    <label for="menu_img" class="fs-5 pb-2">Upload menu Image</label>
                                </div>
                                <div class="col-8">
                                    <input required type="file" name="menu_img" id="menu_img" class="form-control p-3">
                                </div>
                            </div>
                        </div>
                    </div>
